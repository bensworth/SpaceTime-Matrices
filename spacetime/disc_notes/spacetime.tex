...
\textbf{Questions}
\begin{enumerate}
\item Do we really need to explicitly check that mass matrix data is not zero/not smaller than 1e-16? Ahh, is this perhaps because it's assembled with quadrature and maybe some of the integrals evaluated to zero but we don't necessarily know that ahead of time, or maybe who ever implemented  it might be too lazy to compute only non-zero integrals? Also, do we really need to check spatial discretization data too?
\item Boundary conditions for stages? Is this an issue for Galerkin/FEM methods? It's completely non-trivial for FD/FV methods unless using periodic BCs... 
\end{enumerate}
