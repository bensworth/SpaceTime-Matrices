...
def get_UW4_1D(nt, dt, nx, hx, c, u0):
	...
	# Stencil coefficients
	cuu = np.array([-K**3/432*( 9          - 2*K**2), 
	 				-K**2/72*(  3  - 9*K   - 3*K**2  + 2*K**3), 
					 K**2/144*(96 - 45*K  - 24*K**2 + 10*K**3), 
					 1/108*(  108        - 135*K**2 + 45*K**3 + 27*K**4 - 10*K**5), 
					 K**2/144*(96 - 45*K  - 24*K**2 + 10*K**3),
					-K**2/72*(  3  - 9*K   - 3*K**2  + 2*K**3), 
					-K**3/432*( 9          - 2*K**2),
					])
	cuv = np.array([ dt*K/576*(   5          -  4*K**2), 
					-dt*K/864*(  45  + 12*K  - 36*K**2  - 8*K**3), 
					 dt*K/1728*(225 + 384*K - 180*K**2 - 64*K**3), 
					 dt/144*(   144  - 25*K  - 60*K**2 + 20*K**3 + 8*K**4), 
					 dt*K/1728*(225 + 384*K - 180*K**2 - 64*K**3),
					-dt*K/864*(  45  + 12*K  - 36*K**2  - 8*K**3), 
					 dt*K/576*(   5 - 4*K**2)
					])
	cvu = np.array([-K**3/(48*dt)*( 3           - K**2), 
	 				-K**2/(24*dt)*( 2  - 9*K  - 4*K**2 +  3*K**3), 
					 K**2/(48*dt)*(64 - 45*K - 32*K**2 + 15*K**3), 
					-K**2/(12*dt)*(30 - 15*K - 12*K**2 +  5*K**3), 
					 K**2/(48*dt)*(64 - 45*K - 32*K**2 + 15*K**3), 
					-K**2/(24*dt)*( 2  - 9*K  - 4*K**2 +  3*K**3), 
					-K**3/(48*dt)*( 3 - K**2)])
	cvv = np.array([ K/288*(5         - 8*K**2), 
					-K/48*( 5  + 2*K  - 8*K**2  - 2*K**3), 
					 K/96*(25 + 64*K - 40*K**2 - 16*K**3), 
					 1/72*(72 - 25*K - 90*K**2 + 40*K**3 + 18*K**4), 
					 K/96*(25 + 64*K - 40*K**2 - 16*K**3), 
					-K/48*( 5  + 2*K  - 8*K**2  - 2*K**3), 
					 K/288*(5         - 8*K**2)])
