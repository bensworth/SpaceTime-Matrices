     ...
     M(_M), K(_K), b(_b), M_solver(M.GetComm()), z(_M.Height())
{
   ...
   // TODO: Need to figure out how to set Hypre flags here, specifically, to use v
   int distance          =  1;
   std::string prerelax  = "";
   std::string postrelax = "FFC";
   double strength_tol   = 0.1;
   int interp_type       = 100;
   int relax_type        = 0;
   double filterA_tol    = 0.0;
   int splitting         = 6;
   AMG_solver.SetAIROptions();
   // AMG_solver.SetAIROptions(distance, prerelax, postrelax, strength_tol,
   //                          interp_type, relax_type, filterA_tol, splitting);
}
...
void FE_Evolution::ImplicitSolve(const double dt, const Vector &u, Vector &du_dt)
{
   ...
   //    HypreGMRES hGMRES(*T);
   //    hGMRES.SetTol(1e-12);
   //    hGMRES.SetMaxIter(500);
   //    hGMRES.SetPrintLevel(2);
   //    hGMRES.SetPreconditioner(AMG_solver);
   //    std::cout << "dt = " << dt << ", current dt = " << current_dt << "\n";
   // }
