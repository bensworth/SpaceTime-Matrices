...
int main(int argc, char *argv[])
{
  ...
  int ordV = 1;
  int ref_levels = 4;
  const char *petscrc_file = "rc_SpaceTimeKHI_SingAp";
  int verbose = 0;
  int precType = 1;
  int STSolveType = 0;
  int pbType = 1;   
  ...
  // - variables ordering depends on preconditioner used!
  Array<int> offsets(4);
  int idxW, idxP, idxV;
  switch(precType){
    case 0:{
  ...
  // - assemble matrix
  BlockOperator *KHIOp = new BlockOperator( offsets );
  ...
  sol.GetBlock(idxW) = *W0;
  sol.GetBlock(idxP) = *P0;
  sol.GetBlock(idxV) = *V0;
  solPrev = sol;
  ...
  do{ 
    ...
    if ( STSolveType==9 ){
      ...
      khiAssembler->TimeStep( rhs, sol, fname, path, ref_levels, pbType );
    ...
    // otherwise, things get serious
      ...
      if ( verbose == -1 && picardIt == 0 ){
        ...
        khiAssembler->PrintMatrices(path);
      ...
      switch(precType){
        case 0:{
          ...
          temp->SetDiagonalBlock( 0, Mvi );
          temp->SetDiagonalBlock( 1, AAi );
          temp->SetDiagonalBlock( 2, FFi );
          temp->SetBlock( 0, 1, CCC );
          temp->SetBlock( 1, 2, Mwp );
          KHIPr = temp;
          break;
        }
        case 1:{
      ...
      if (myid == 0){
        ...
        if( output>0 ){
          ...
          khiAssembler->GetMeshSize( hmin, hmax, kmin, kmax );
    ...
    if( output>2 ){
      ...
      string outFileName = "STKHI_" + pbName;
      if (picardIt == 0){
        khiAssembler->SaveExactSolution( outFilePath, outFileName+"_Ex" );
      }
      if ( pbType > 10 ){
        outFileName += "_it" + to_string(picardIt);
      }
      int colsV[2] = { myid*(MMv->NumRows()), (myid+1)*(MMv->NumRows()) };
      int colsP[2] = { myid*(AAA->NumRows()), (myid+1)*(AAA->NumRows()) };
      int colsW[2] = { myid*(FFF->NumRows()), (myid+1)*(FFF->NumRows()) };

      HypreParVector wh( MPI_COMM_WORLD, numProcs*(FFF->NumRows()), sol.GetBlock(idxW).GetData(), colsW ); 
      HypreParVector ph( MPI_COMM_WORLD, numProcs*(AAA->NumRows()), sol.GetBlock(idxP).GetData(), colsP ); 
      HypreParVector vh( MPI_COMM_WORLD, numProcs*(MMv->NumRows()), sol.GetBlock(idxV).GetData(), colsV ); 

      stokesAssembler->SaveSolution( wh, ph, vh, outFilePath, outFileName );
    ...
    if ( pbType > 10 ){
      ...
      khiAssembler = new KHISTOperatorAssembler( MPI_COMM_WORLD, mesh_file, ref_levels, ordW, ordP, ordV,
                                                 dt, mu, Pe, fFun, gFun, nFun,
