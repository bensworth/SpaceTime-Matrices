...
// this is useful to include BC for fespaces with multiple components
void BlockNonlinearForm::SetEssentialTrueDofs(
   const Array<Array<int> *> &essTdofs, Array<Vector *> &rhs)
{
   for (int s = 0; s < fes.Size(); ++s)
   {
      *(ess_tdofs[s]) = *(essTdofs[s]); // just copy the nodes given
